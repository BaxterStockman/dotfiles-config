...
if v:version >= 703
    ...
    function! SourceLocal(...)
        ...
        elseif exists('g:rcdir')
            let rcdir = g:rcdir
        elseif exists('g:use_xdg_layout') && g:use_xdg_layout
            call InitializeXDGLayout()
            let rcdir = $XDG_CONFIG_HOME . "/vim"
        else
            let rcdir = "~/.vimrc.d"
        endif
...
function! MakePath(path, ...)
  if exists("*mkdir")
    if !isdirectory(a:path)
      call call('mkdir', [a:path] + a:000)
    endif
  endif

  if !isdirectory(a:path)
    echo "Warning: Unable to create path: " . a:path
    echo "Try: mkdir -p " . a:path
    return 0
  endif
...
function! InitializeXDGLayout()
  let xdg_layout = {
    \ '$XDG_CONFIG_HOME': '~/.config',
    \ '$XDG_CACHE_HOME': '~/.cache',
    \ '$XDG_DATA_HOME': '~/.local/share' }

  for [env_var, path] in items(xdg_layout)
    if !exists(env_var)
      let fullpath = simplify(expand(path))
      exec "let " . env_var . '="' .  escape(fnameescape(fullpath), '"') . '"'
    endif

    call MakePath(expand(env_var), 'p')
  endfor
endfunction

let g:use_xdg_layout = 1
" Adapted from spf13: https://github.com/spf13/spf13-vim/blob/3.0/.vimrc
function! InitializeDirectories()
  ...
  for [dirname, settingname] in items(dir_list)
    let directory = simplify(expand(parent . dirname . '/'))
    if MakePath(directory, 'p')
